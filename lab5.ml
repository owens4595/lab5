...
let validated_rgb (c : color) : color =
  let check (num : int) : bool =
    num >= 0 && num <= 255 in
  match c with 
  | RGB (r, g, b) -> if check r && check g && check b then c
                     else raise (Invalid_color "some input out of range")
  | _ -> c ;;
...
let convert_to_rgb (c : color) : int * int * int =
  ...
  | RGB x -> x 
  | Simple c ->
    if c = Red then (255, 0, 0)
    else if c = Crimson then (164, 16, 52)
    else if c = Orange then (255, 165, 0)
    else if c = Yellow then (255, 255, 0)
    else if c = Green then (0, 255, 0)
    else if c = Blue then (0, 0, 255)
    else if c = Indigo then (75, 0, 130)
    else (240, 130, 240) ;;
